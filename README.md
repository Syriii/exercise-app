...
尚未产生内容的目录不会提前创建。餐食照片、个人记录、数据库、备份和访问凭证属于私有运行数据，不进入本仓库。已上传照片与识别、修改过程长期关联保留，直到主动删除；结构化餐食不依赖原图存在。照片支持账号内查看和下载，但默认数据库备份不含图片字节，需另行规划媒体备份。详见[使用过程留存](docs/domain/meal-usage-history.md)。APK 和 AAB 作为发布产物也不直接提交到 Git。
