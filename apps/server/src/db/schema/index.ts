...
export const trainingSessions = pgTable(
  "training_sessions",
  {
    ...
    note: text("note"),
    recordedTime: text("recorded_time"),
    recordWrite: jsonb("record_write").$type<{ hash: string; baseRevision: number; modes?: Record<string, "sets" | "activity" | "count" | "unknown" | "mixed" | null | undefined> }>(),
    deletedAt: timestamp("deleted_at", { withTimezone: true }),
...
export const personalProfiles = pgTable(
  "personal_profiles",
  {
    ...
    setupProgress: jsonb("setup_progress").$type<{ profile: boolean; measurement: boolean; strategy: boolean; completed: boolean }>(),
    revision: integer("revision").default(1).notNull(),
    ...timestamps,
  },
  (table) => [
