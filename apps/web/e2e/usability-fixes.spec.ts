...
test('completion saves all draft actions; a lost response can be retried without duplication', async ({ page }) => {
  ...
  await expect(page.locator('.history-session details')).toContainText('12 / 12 次');
  await expect(page.locator('.history-session details')).toContainText('拉伸 · 数量未记录');
});
...
test('food draft survives tab navigation; account actions are discoverable and logout clears drafts', async ({ page }) => {
  const profile = await page.request.put('/api/v1/planning/profile', {data: {
    revision:(await (await page.request.get('/api/v1/planning/profile')).json()).revision,birthDate:null,sexCategory:null,heightCm:null,palCategory:null,
