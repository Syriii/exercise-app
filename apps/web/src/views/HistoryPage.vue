...
<template>
  <AppShell page-class="history-page" rail-note="记录回到原页面修改，趋势只采用实际记录。">
    <header class="view-header"><h1>历史</h1><nav class="history-filters" aria-label="历史视图"><button :aria-pressed="tab === 'records'" @click="tab = 'records'"><AppIcon name="list" />记录</button><button :aria-pressed="tab === 'trends'" @click="tab = 'trends'"><AppIcon name="history" />趋势</button></nav></header>
    ...
    <template v-else-if="tab === 'records'">
      ...
      <p v-if="!days.length && !error">这段时间没有符合筛选的记录。</p>
      ...
      <section v-for="day in days" :key="day" class="work-panel history-day-records" :aria-label="day + '的记录'">
        <h2>{{ day }}</h2>
        <template v-if="filter === 'all' || filter === 'training'"><article v-for="record in sessions.filter(value => value.localDate === day)" :key="record.id" class="history-session"><strong><AppIcon name="train" />训练 · {{ record.recordedTime ?? '时间未记录' }}</strong><p>{{ record.items.filter(item => item.status === 'completed').map(item => item.performedExerciseName ?? item.exerciseName).join('、') || '没有已确认的实际动作' }}</p><details><summary>查看记录内容</summary><ul><li v-for="item in record.items.filter(item => item.status === 'completed')" :key="item.id">{{ item.performedExerciseName ?? item.exerciseName }} · {{ actionSummary(item) }}</li></ul><p>{{ record.note }}</p></details><button class="text-action" @click="openRecord('training', record.id, day)">查看／修改训练</button></article></template>
        <template v-if="filter === 'all' || filter === 'nutrition'"><article v-for="meal in meals.filter(value => value.localDate === day)" :key="meal.id" class="history-session"><strong><AppIcon name="food" />{{ meal.name ?? '餐食' }} · {{ new Date(meal.occurredAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }) }}</strong><p>{{ meal.contributions.map(value => value.label).join('、') || '尚无食物明细' }}</p><button class="text-action" @click="openRecord('nutrition', meal.id, day)">查看／修改餐食</button></article></template>
        <template v-if="filter === 'all' || filter === 'measurement'"><article v-for="item in body.filter(value => value.localDate === day)" :key="item.id" class="history-session"><strong><AppIcon name="scale" />体重 {{ item.weightKg }} kg</strong><p>腰围 {{ item.waistCm === null ? '未记录' : item.waistCm + ' cm' }} · {{ item.note ?? '' }}</p><button class="text-action" @click="openRecord('measurement', item.id, day)">查看／修正身体记录</button></article></template>
      </section>
    </template>
    <template v-else>
      ...
      <section v-if="metric === 'body'" class="work-panel"><h2>身体趋势</h2><details><summary>数据说明</summary><p>仅列出真实测量日期，不补零或推算未测日期。</p></details><p v-if="!bodyAvailable">身体数据暂时不可用。</p><p v-else-if="!body.length">这段时间没有身体测量记录。</p><BodyTrendChart v-if="bodyAvailable && body.length" :measurements="body" /><ul v-if="bodyAvailable && body.length" class="measurement-list"><li v-for="item in body" :key="item.id"><div><strong>{{ item.localDate }}</strong><span>体重 {{ item.weightKg }} kg · 腰围 {{ item.waistCm === null ? '未记录' : item.waistCm + ' cm' }}</span></div><button class="text-action" @click="openRecord('measurement', item.id, item.localDate)">查看／修正</button></li></ul></section>
      <section v-else-if="metric === 'training'" class="work-panel"><h2>训练趋势</h2><p v-if="!trainingAvailable">训练记录暂时不可用。</p><template v-else><dl class="body-summary"><div><dt>训练记录</dt><dd>{{ actual.length }} <small>次</small></dd></div><div><dt>训练天数</dt><dd>{{ trainingDays }} <small>天</small></dd></div></dl><label>查看某个动作<select v-model="exercise"><option value="">请选择动作</option><option v-for="name in names" :key="name">{{ name }}</option></select></label><details><summary>统计说明</summary><p>次数按实际训练记录计。同一动作逐次比较；未记录的数量不当作零，不混合不同动作计算进步。</p></details><ul class="measurement-list"><li v-for="row in actionRows" :key="row.id"><div><strong>{{ row.date }} · {{ exercise }}</strong><span>{{ row.text }}</span></div><button class="text-action" @click="openRecord('training', row.recordId, row.date)">查看训练</button></li></ul></template></section>
      <section v-else class="work-panel"><h2>饮食趋势</h2><p class="field-help">仅统计已记录餐食，不代表全天摄入。</p><details><summary>统计说明</summary><p>不计算可能漏记日期的平均摄入；缺少的日期不补零。</p></details><p v-if="!mealsAvailable">饮食记录暂时不可用。</p><p v-else-if="!mealDates.length">这段时间没有餐食记录。</p><div v-else class="history-trend-table-wrap" tabindex="0" aria-label="饮食趋势，可左右滚动"><table class="history-trend-table"><thead><tr><th>日期</th><th v-for="n in nutrients" :key="n.key">{{ n.label }}</th></tr></thead><tbody><tr v-for="day in mealDates" :key="day"><th scope="row">{{ day }}</th><td v-for="n in nutrients" :key="n.key" :data-label="n.label">{{ nutrientText(day, n.key, n.unit) }}</td></tr></tbody></table></div></section>
    </template>
  </AppShell>
</template>
<style scoped>
